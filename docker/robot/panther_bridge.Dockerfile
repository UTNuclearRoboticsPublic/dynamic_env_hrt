...
RUN git clone -b ros1 https://github.com/husarion/panther_msgs.git
WORKDIR /ros1_ws 
RUN source /opt/ros/${ROS1_DISTRO}/setup.bash && catkin_make
...
colcon build --packages-select ar_track_alvar_msgs
...
COPY ./panther_bridge_entrypoint.sh /